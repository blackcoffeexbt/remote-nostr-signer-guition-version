...
void handleConnect(DynamicJsonDocument &doc, String &requestingNpub)
{
  ...
  // send ack
  String responseMsg = "";
  // if secret is not empty, respond with result = "secret", else "ack"
  if(secret.length() > 0) {
    responseMsg = "{\"id\":\"" + requestId + "\",\"result\":\"" + secret + "\"}";
  } else {
    responseMsg = "{\"id\":\"" + requestId + "\",\"result\":\"ack\"}";
  }
  
  _logToSerialWithTitle("responseMsg is: ", responseMsg);
  String dm = nostr::getEncryptedDm(nsecHex, npubHex, requestingNpub.c_str(), 24133, unixTimestamp, responseMsg, "nip44");
...
void handleSignEvent(DynamicJsonDocument &doc, char const *requestingPubKey)
{
  ...
  // _logToSerialWithTitle("responseMsg is: ", responseMsg);
  String dm = nostr::getEncryptedDm(nsecHex, npubHex, requestingPubKey, 24133, timestamp, String(responseMsgChar), "nip44");
...
void signerStartedConnectedEvent(WStype_t type, uint8_t *payload, size_t length)
{
  ...
  case WStype_CONNECTED:
  {
    ...
    String dm = nostr::getEncryptedDm(nsecHex, npubHex, npubHex, 4, unixTimestamp, message, "nip44");
...
void handlePing(DynamicJsonDocument &doc, const char *requestingNpub)
{
  ...
  String dm = nostr::getEncryptedDm(nsecHex, npubHex, requestingNpub, 24133, unixTimestamp, responseMsg, "nip44");
  webSocket.sendTXT(dm);
}
...
void handleNip04Encrypt(DynamicJsonDocument &doc, const char *requestingNpub)
{
  ...
  String dm = nostr::getEncryptedDm(nsecHex, npubHex, requestingNpub, 24133, unixTimestamp, responseMsg, "nip04");
  webSocket.sendTXT(dm);
}
...
void handleSigningRequestEvent(uint8_t *data)
{
  ...
  // does the data include a ?iv ?
  String message = "";
  if (String((char *)data).indexOf("?iv=") != -1) {
    Serial.println("data includes ?iv= so it's a NIP04 message");
    message = nostr::nip04Decrypt(nsecHex, String((char *)data));
  } else {
    Serial.println("data has no ?iv= so it's a nip44 message");
    message = nostr::nip44Decrypt(nsecHex, String((char *)data));
  ...
  {
    Serial.println("REQUESTED nip44_encrypt");
    showMessage("Requested nip44_encrypt", "Not implemented yet");
  }
  else if (method == "nip44_decrypt")
  {
    Serial.println("REQUESTED nip44_decrypt");
    showMessage("Requested nip44_decrypt", "Not implemented yet");
  }
  else
  {
...
void setup()
{
  ...
  {
    showMessage("Formatting storage", "This may take a while. Please wait...");
